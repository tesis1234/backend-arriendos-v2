...
const getRentalRequests = async (ownerId) => {
  const [rows] = await db.query(
    ...
    SELECT 
      ...
      a.id_arriendo AS id_arriendo   -- 🔥🔥 EL DATO QUE FALTABA

    FROM notificaciones n

    JOIN propiedades p 
      ON JSON_UNQUOTE(JSON_EXTRACT(n.data, '$.propertyId')) = p.id_propiedad

    JOIN usuarios u 
      ON u.id_usuario = JSON_UNQUOTE(JSON_EXTRACT(n.data, '$.tenantId'))

    LEFT JOIN solicitudes_arriendo sa 
      ON sa.id_estudiante = u.id_usuario 
      AND sa.id_propiedad = p.id_propiedad

    LEFT JOIN habitaciones h 
      ON h.id_habitacion = sa.id_habitacion

    -- 🔥 JOIN CLAVE PARA TRAER EL ARRIENDO CREADO
    LEFT JOIN arriendos a
      ON a.id_estudiante = u.id_usuario
      AND a.id_propiedad = p.id_propiedad
      AND (a.id_habitacion = sa.id_habitacion OR sa.id_habitacion IS NULL)

    JOIN propietarios pr 
      ON p.id_propietario = pr.id_propietario

    JOIN usuarios up 
      ON pr.id_propietario = up.id_usuario

    WHERE p.id_propietario = ?
      AND (sa.estado IN ('pendiente', 'aceptado') OR sa.estado IS NULL)
